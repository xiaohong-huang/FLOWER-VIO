...
void SWFOptimization::AddFactors(std::vector<VisualInertialBase*>& visual_inertial_bases, std::set<int>factor_index_set) {
    ...
    for (int j = 1; j < image_count; j++) {
        ...
        std::vector<double*>parameters{para_pose[j - 1], para_speed_bias[j - 1],
                                       ...
                                       &scale_factor};
        if (ESTIMATE_GLOBAL_POSE)parameters.push_back(para_global_pos);
...
#if USE_REPARAM
        flower_vio::MatrixXd lhs2 = flower_vio::MatrixXd::Zero(lhs.cols() + 6 * (SWF_SIZE_IN - 1), lhs.cols() + 6 * (SWF_SIZE_IN - 1));
...
void SWFOptimization::set_last_marg_info_idx(int idx0) {
    ...
    if (last_marg_info) {
        ...
        last_marg_info->gyr0 = pre_integrations[idx0]->gyrj;
        ...
        ASSERT(count1 == 1 && count2 == (int)last_marg_info->p_is.size() && last_marg_info->v_i >= 0 && (last_marg_info->extrinsic_i >= 0 || !ESTIMATE_EXTRINSIC) && last_marg_info->scale_factor_i >= 0 && (last_marg_info->R_WI_WC_i >= 0||!ESTIMATE_GLOBAL_POSE));;
