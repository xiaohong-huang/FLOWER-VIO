...
inline flower_vio::MatrixXd InvertPSDMatrix(const flower_vio::MatrixXd& m) {
    return m.template selfadjointView<Eigen::Upper>().llt().solve(flower_vio::MatrixXd::Identity(m.rows(), m.rows()));
}
