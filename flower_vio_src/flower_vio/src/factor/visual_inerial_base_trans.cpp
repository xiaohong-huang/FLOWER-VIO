...
void VisualInertialBase::BuildForwardMatrixInfo() {
    ...
    flower_vio::MatrixXd idepth_pos1 = flower_vio::MatrixXd::Zero(deliver_idepth_pointer.size(), 12);
    flower_vio::MatrixXd idepth_pos0 = flower_vio::MatrixXd::Zero(deliver_idepth_pointer.size(), 6);
    ...
    for (auto& it : long_feature_factors) {
        ...
        if (deliver_idepth_pointer.find(p_idepthi) != deliver_idepth_pointer.end()) {
            ...
            if (pindexs[0] == 0) {
                ...
                matrix_info.push_back(MatirxInfo(idepthi_p_local, idepthi_p_local, flower_vio::Vector1d(&idepth_tmp), false, 1, 1)); //D^-1
...
void VisualInertialBase::UpdateLhsRhsGradientNew() {
    ...
    if (nonlinear_quantity_deliver_accum > NLQ_THRESHOLD) {
        ...
        if (O_BIAS0 != O_POSR) {
            ...
            flower_vio::MatrixXd Amm = lhs_outside.block(0, 0, m, m);
            flower_vio::MatrixXd Amn = lhs_outside.block(0, m, m, n);
            flower_vio::MatrixXd Anm_Amm_inverse(n, m);
        ...
        if (num_alone) {
            ...
            flower_vio::MatrixXd Amm = lhs_outside.block(n + k, n + k, m, m);
        ...
        if (deliver_idepth_pointer.size()) {
            ...
            flower_vio::MatrixXd A = lhs_outside.block(idx, idx, size, size);
            flower_vio::MatrixXd D = A;
            int count = 0;
            int size2 = 0;
...

        {
            ...
            flower_vio::MatrixXd Amm = lhs_outside.block(k, k, m, m);
...
void VisualInertialBase::UpdateGaussAndGradientOutside() {
    ...
    if (!next_vibase) {
        ...
        flower_vio::MatrixXd A =  lhs_outside.block(k + m, k + m, n, n);
...
void VisualInertialBase::UpdateStateNew(double gauss_scale, double gradient_scale) {
    ...
    Eigen::Map<flower_vio::VectorXd>inc_new(inc_outside_pointer, gradient_outside.size());
...
void VisualInertialBase::GetMatrixNext(flower_vio::MatrixXd& lhs, Eigen::VectorXd& rhs, std::vector<double*>& new_parameters, std::vector<int>& new_sizes) {
