...
void VisualInertialBase::MargeRhs() {
    ...
    for (auto it1 = idepth_map_factors_short.begin(); it1 != idepth_map_factors_short.end(); it1++) {
        ...
        for (auto& pindexi : visual_factor->pindexs_set) {
            uint8_t porderi = pindex2order[pindexi];
            Eigen::Map<flower_vio::Vector6d>(rhs_posbias_pointer + order2p_local[porderi]) -= flower_vio::Vector6d(lhs_pos_idepth + 6 * (pindexi - min_pindex)) * (lhs_idepth[0] * rhs_idepth[0]);
        }
    }
    ...
    for (int marg_orderi = O_MARG_POS; marg_orderi < O_IDEPTHR; ++marg_orderi) {
        ...
        if (marg_orderi >= O_POSR) Eigen::Map<flower_vio::VectorXd>(rhs_outside.data() + outside_pindexi, sizei) = Eigen::Map<flower_vio::VectorXd>(rhs_pos_biasi, sizei);
        else Eigen::Map<flower_vio::VectorXd>(rhs_margpos_pointer + marg_pindexi, sizei) = Eigen::Map<flower_vio::VectorXd>(rhs_pos_biasi, sizei);
    }
...
void VisualInertialBase::MargRhsNew() {
    ...
    if (O_BIAS0 != O_POSR) {
        ...
        flower_vio::MatrixXd Amn = lhs_outside.block(0, m, m, n);
        MatrixTransposeVectorMultiply < Eigen::Dynamic, Eigen::Dynamic, -1 > (
            Amn.data(),  m, n,
            rhs_outside.data() + 0,
            rhs_outside.data() + m);
    ...
    if (num_alone) {
        ...
        flower_vio::MatrixXd Amn = lhs_outside.block(k, n + k, n, m).transpose();
        MatrixTransposeVectorMultiply < Eigen::Dynamic, Eigen::Dynamic, -1 > (
            Amn.data(),  m, n,
...

    {
        ...
        flower_vio::MatrixXd Amn = lhs_outside.block(k, k + m, m, n);
        MatrixTransposeVectorMultiply < Eigen::Dynamic, Eigen::Dynamic, -1 > (
            Amn.data(),  m, n,
            rhs_outside.data() + k,
...
void VisualInertialBase::EvaluateNonlinearQuantity() {
    if (history_flag) {
        ...
        nonlinear_quantity += (Eigen::Map<flower_vio::VectorXd>(visual_residuals, (visual_residual_long_num + visual_residual_short_num) * 2) +
                               Eigen::Map<flower_vio::VectorXd>(visual_model_residual_accum, (visual_residual_long_num + visual_residual_short_num) * 2) -
                               Eigen::Map<flower_vio::VectorXd>(visual_residuals_linerized, (visual_residual_long_num + visual_residual_short_num) * 2)).squaredNorm();
