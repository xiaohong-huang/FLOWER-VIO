...
#define SQRT_INFO_SCALE (100000.0)
...
void VisualInertialBase::EvaluateLastMargInfo() {
    ...
    if (!have_hist) {
        ...
        double* parameter0 = order2pointers[O_POS0];
#else
        ...
        flower_vio::Matrix6d yaw_constraint_lhs;
        flower_vio::Vector6d yaw_constraint_rhs;
...
void VisualInertialBase::EvaluatePriorAlpha(double* alpha1) {
    ...
    int m = last_marg_info->m;
    flower_vio::VectorXd gradient_prior(n);
    ...
    for (int i = 0; i < static_cast<int>(keep_block_size.size()); ++i) {
        ...
        int idxi = keep_block_idx[i] - m;
        int idxi2 = prior_idx[i];
        gradient_prior.segment(idxi, sizei) = Eigen::Map<flower_vio::VectorXd>(gradient_outside.data() + idxi2 - shift1, sizei);
    }
#if USE_REPARAM
    Eigen::VectorXd tmp;
    ...
    if (!have_hist) {
#if !ESTIMATE_GLOBAL_POSE
        flower_vio::VectorXd model_residuals = yaw_constraint_jacobian * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[O_POS0]);
#else
        flower_vio::VectorXd model_residuals = yaw_constraint_jacobian * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[O_POSGLOBAL]);
#endif
...
void VisualInertialBase::EvaluatePriorModelCostChange(double* model_cost_change) {
    ...
    int n = last_marg_info->n;
    int m = last_marg_info->m;
    flower_vio::VectorXd inc_prior(n);
    inc_prior.setZero();
    int shift1 = order2p_local[O_POSR];
    for (int i = 0; i < static_cast<int>(keep_block_size.size()); ++i) {
        ...
        int idxi = keep_block_idx[i] - m;
        int idxi2 = prior_idx[i];
        inc_prior.segment(idxi, sizei) = Eigen::Map<flower_vio::VectorXd>(inc_outside_pointer + idxi2 - shift1, sizei);
    }
    ...
    flower_vio::VectorXd model_residuals = last_marg_info->linearized_jacobiansF * tmp;
#else
    flower_vio::VectorXd model_residuals = last_marg_info->linearized_jacobians * inc_prior;
#endif
    model_cost_change[0] += model_residuals.transpose() * ( prior_residual_save - model_residuals / 2);

    if (!have_hist) {
#if !ESTIMATE_GLOBAL_POSE
        flower_vio::VectorXd model_residuals = yaw_constraint_jacobian * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[O_POS0]);
#else
        flower_vio::VectorXd model_residuals = yaw_constraint_jacobian * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[O_POSGLOBAL]);
#endif
    ...
    if (fix_scale && !have_hist) {
        ...
        for (int pindexi = 0; pindexi < pos_para_count - 1; pindexi++) {
            int orderi = pindex2order[pindexi];
            ASSERT(order2p_local[orderi] - order2p_local[O_POSR] >= 0);
            model_residuals += (init_distance_constraint_jacobian.segment(pindexi * 3, 3).transpose() * flower_vio::Vector3d(inc_outside_pointer + order2p_local[orderi] - shift1))(0);
        }
...
void VisualInertialBase::AddIMUFactor(IMUPreFactor* imu_pre_factor) {
    ...
    int p_idx = 5;
    if (ESTIMATE_GLOBAL_POSE) {
        if (global_pos_pointer)ASSERT(global_pos_pointer == imu_pre_factor->parameters[p_idx]);
        p_idx++;
    }
    if (ESTIMATE_TD) {
...
void VisualInertialBase::PrepareMatrix() {
    ...
    imu_jacobians_in[0] = flower_vio::Matrix15_6d();
    imu_jacobians_in[1] = flower_vio::Matrix15_9d();
    imu_jacobians_in[2] = flower_vio::Matrix15_6d();
    imu_jacobians_in[3] = flower_vio::Matrix15_9d();
    imu_jacobians_in[4] = flower_vio::Vector15d ();
    int p_idx2 = 5;
    if (ESTIMATE_GLOBAL_POSE) {
        imu_jacobians_in[p_idx2] = flower_vio::Matrix15_6d (); p_idx2++;
    }
    if (ESTIMATE_TD) {
        imu_jacobians_in[p_idx2] = flower_vio::Vector15d (); p_idx2++;
    }
    if (ESTIMATE_EXTRINSIC) {
        imu_jacobians_in[p_idx2] = flower_vio::Matrix15_6d (); p_idx2++;
    }
    if (ESTIMATE_ACC_SCALE) {
        imu_jacobians_in[p_idx2] = flower_vio::Matrix15_3d (); p_idx2++;
    }
    if (ESTIMATE_GYR_SCALE) {
        imu_jacobians_in[p_idx2] = flower_vio::Matrix15_3d (); p_idx2++;
    }
...
void VisualInertialBase::FactorUpdates() {
    ...
    int p_idx = 0;
    if (ESTIMATE_GLOBAL_POSE) {
        O_POSGLOBAL = para_count + 1 + p_idx; p_idx++;
    }

    if (ESTIMATE_TD) {
        O_TD = para_count + 1 + p_idx; p_idx++;
    }
    if (ESTIMATE_EXTRINSIC) {
        O_EXTRINSIC = para_count + 1 + p_idx; p_idx++;
    }
    if (ESTIMATE_ACC_SCALE) {
        O_ACC_S = para_count + 1 + p_idx; p_idx++;
    }
    if (ESTIMATE_GYR_SCALE) {
        O_GYR_S = para_count + 1 + p_idx; p_idx++;
    }

    O_IDEPTHR = para_count + 1 + p_idx;
    O_FULL = para_count + idepth_long_count + 1 + p_idx;
    ...
    int r_size = order2p_local[O_FULL] - order2p_local[O_POSR];
    gauss_outside = flower_vio::VectorXd(r_size);
    lhs_outside = flower_vio::MatrixXd(r_size, r_size);
    rhs_outside = flower_vio::VectorXd(r_size);
    gradient_outside = flower_vio::VectorXd(r_size);
...
void VisualInertialBase::VisualJacobianResidualUpdatelhsRhs(uint8_t pindexi, uint8_t pindexj, Eigen::Vector3d pts_i, Eigen::Vector3d pts_j,
    ...
    Eigen::Map<flower_vio::Matrix2_6d>visual_jacobians_in1(jacobian_pointer);
    Eigen::Map<flower_vio::Matrix2_6d>visual_jacobians_in2(jacobian_pointer + 6 * 2);
    Eigen::Map<flower_vio::Vector2d>visual_jacobians_in3(jacobian_pointer + 6 * 4);
    Eigen::Map<flower_vio::Vector2d>visual_residual_in(visual_residual_raw_in);

    {
        ...
        flower_vio::Matrix2d visual_scale =
            sqrt_rho1_ * (flower_vio::Matrix2d::Identity() - alpha_sq_norm_ * visual_residual_in * visual_residual_in.transpose());
    ...
    if (need_update_pos) {
        ...
        Eigen::Map<flower_vio::Vector6d>(rhs_posbias_pointer + order2p_local[porderi]) += Eigen::Map<flower_vio::Vector6d>(tmp);
        Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[porderi]) += Eigen::Map<flower_vio::Vector6d>(tmp);
    ...
    if (need_update_pos) {
        ...
        Eigen::Map<flower_vio::Vector6d>(rhs_posbias_pointer + order2p_local[porderj]) += Eigen::Map<flower_vio::Vector6d>(tmp);
        Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[porderj]) += Eigen::Map<flower_vio::Vector6d>(tmp);
    }
...
void VisualInertialBase::IMUJacobianResidualUpdatelhsRhs() {
    ...
    for (auto it = imu_map_factors.begin(); it != imu_map_factors.end(); ++it) {
        ...
        for (int i = 0; i < (int)orders.size(); ++i) {
            ...
            Eigen::Map<flower_vio::VectorXd>(rhs_posbias_pointer + order2p_local[orderi], sizei) += Eigen::Map<flower_vio::VectorXd>(tmp, sizei);
            Eigen::Map<flower_vio::VectorXd>(rhs_posbias_save_pointer + order2p_local[orderi], sizei) += Eigen::Map<flower_vio::VectorXd>(tmp, sizei);
...
void VisualInertialBase::MargIdepth(const std::set<uint8_t>& pindexs_set,
    ...
    for (auto& pindexi : pindexs_set) {
        ...
        flower_vio::Vector6d Anm_Amm_inverse = flower_vio::Vector6d(lhs_pos_idepth + 6 * (pindexi - min_pindex)) * lhs_idepth[0];

        Eigen::Map<flower_vio::Vector6d>(rhs_posbias_pointer + order2p_local[porderi]) -= Anm_Amm_inverse * rhs_idepth[0];
...
void VisualInertialBase::MargInsideBias() {
    ...
    for (int marg_order = 0; marg_order < E_O_MARG_BIAS; ++marg_order) {
        ...
        flower_vio::Matrix9d inverse_lhs =
            InvertPSDMatrix(Eigen::Map<flower_vio::Matrix9d>(lhs_posbias[lhs_posbias_indexm + marg_order]) );
        ...
        for (int j = 0; j < (int)connect_orders.size(); ++j) {
            ...
            flower_vio::MatrixX9d Anm_Amm_inverse;
            int lhs_posbias_indexj = orderj * O_IDEPTHR;

            Anm_Amm_inverse = flower_vio::MatrixX9d(sizej, 9);
            ...
            Eigen::Map<flower_vio::MatrixXd>(lhs_posbias[lhs_posbias_indexm + orderj], 9, sizej) = Anm_Amm_inverse.transpose();
        }
    }
}
...
void VisualInertialBase::MargInsidePos() {
    ...
    int r_size = order2p_local[O_FULL] - order2p_local[O_POSR];
    int mpos_size = order2p_local[O_POSR] - order2p_local[O_MARG_POS];
    Eigen::Map<flower_vio::MatrixXd>lhs_margpos_outside(lhs_margpos_outside_pointer, mpos_size, r_size);
    Eigen::Map<flower_vio::MatrixXd>lhs_margpos(lhs_margpos_pointer, mpos_size, mpos_size);
    ...
    for (int marg_orderi = O_MARG_POS; marg_orderi < O_IDEPTHR; ++marg_orderi) {
        ...
        if (marg_orderi >= O_POSR)
            Eigen::Map<flower_vio::VectorXd>(rhs_outside.data() + outside_pindexi, sizei) = Eigen::Map<flower_vio::VectorXd>(rhs_pos_biasi, sizei);
        else
            Eigen::Map<flower_vio::VectorXd>(rhs_margpos_pointer + marg_pindexi, sizei) = Eigen::Map<flower_vio::VectorXd>(rhs_pos_biasi, sizei);
        ...
        for (int marg_orderj = marg_orderi; marg_orderj < O_IDEPTHR; ++marg_orderj) {
            ...
            if (marg_orderi >= O_POSR && marg_orderj >= O_POSR)
                lhs_outside.block(outside_pindexi, outside_indexj, sizei, sizej) = Eigen::Map<flower_vio::MatrixXd>(lhs_pos_biasij, sizei, sizej);
            else if (marg_orderi < O_POSR && marg_orderj < O_POSR)
                lhs_margpos.block(marg_pindexi, marg_indexj, sizei, sizej) = Eigen::Map<flower_vio::MatrixXd>(lhs_pos_biasij, sizei, sizej);
            else if (marg_orderi < O_POSR && marg_orderj >= O_POSR)
                lhs_margpos_outside.block(marg_pindexi, outside_indexj, sizei, sizej) = Eigen::Map<flower_vio::MatrixXd>(lhs_pos_biasij, sizei, sizej);
        }
    }
    ...
    for (auto& it : long_feature_factors) {
        ...
        for (auto& pindexi : pindexs_set) {
            ...
            if (outside_pindexi >= 0)
                lhs_outside.block(outside_pindexi, findex, 6, 1) = Eigen::Map<flower_vio::Vector6d>(lhs_pos_idepth + 6 * (pindexi - min_pindex));
            else
                lhs_margpos_outside.block(marg_pindexi, findex, 6, 1) = Eigen::Map<flower_vio::Vector6d>(lhs_pos_idepth + 6 * (pindexi - min_pindex));
        }
        findex++;
    }
    ...
    if (sizem > 0) {
        ...
        flower_vio::MatrixXd Anm_Amm_inverse(sizen, sizem);
        ...
        MatrixVectorMultiply < Eigen::Dynamic, Eigen::Dynamic, -1 > (
            ...
            rhs_margpos_pointer,
            rhs_outside.data());
        Eigen::Map<flower_vio::MatrixXd>(lhs_margpos_outside_pointer, sizem, sizen) = Anm_Amm_inverse.transpose();
    }
...

    {
        for (int marg_orderi = O_POSR; marg_orderi < O_IDEPTHR; ++marg_orderi) {
            int sizei = order2size[marg_orderi];
            int outside_pindexi = order2p_local[marg_orderi] - shift2;
            gradient_outside.segment(outside_pindexi, sizei) = Eigen::Map<flower_vio::VectorXd>(rhs_posbias_save_pointer + order2p_local[marg_orderi], sizei);
        }
...
void VisualInertialBase::UpdateMargposUseGradientAndGauss(double gradient_scale, double gauss_scale) {
    ...
    for (int order = O_MARG_POS; order < O_POSR; ++order) {
        Eigen::Map<flower_vio::Vector3d> P(order2pointers[order]);
        ...
        P -= Eigen::Map<flower_vio::Vector3d>(inc_posbias_pointer + order2p_local[order]);
        Q = (  Q *  Utility::deltaQ(-Eigen::Map<flower_vio::Vector3d>(inc_posbias_pointer + order2p_local[order] + 3))  ).normalized();
    }
}
...
void VisualInertialBase::UpdateMargBiasUseGradientAndGauss(double gradient_scale, double gauss_scale) {
    for (int marg_order = O_MARG_POS - 1; marg_order >= 0; marg_order--) {
        Eigen::Map<flower_vio::Vector9d> B(order2pointers[marg_order]);
        ...
        for (int i = 0; i < 9; ++i)
            *pointer3++ = gauss_scale * (*pointer1++) - gradient_scale * (*pointer2++);
        B -=  Eigen::Map<flower_vio::Vector9d>(inc_posbias_pointer + order2p_local[marg_order]);
    }
}
...
void VisualInertialBase::SaveGaussStep() {
    ...
    if (idepth_map_factors_short.size())
        Eigen::Map<flower_vio::VectorXd>(gauss_idepth_short_pointer, idepth_map_factors_short.size()) =
            Eigen::Map<flower_vio::VectorXd>(inc_idepth_short_pointer, idepth_map_factors_short.size());
}
...
void VisualInertialBase::UpdateMargposGaussStep() {
    ...
    if (mpos_size == 0)
        return;
    flower_vio::VectorXd margpos_inc(mpos_size);
...
void VisualInertialBase::SaveGaussStepOutside() {
    int shift1 = order2p_local[O_POSR];
    Eigen::Map<flower_vio::VectorXd>(inc_outside_pointer, order2p_local[O_FULL] - order2p_local[O_POSR]) =
        Eigen::Map<flower_vio::VectorXd>(gauss_outside.data(), order2p_local[O_FULL] - order2p_local[O_POSR]);
...
void VisualInertialBase::EvaluateAlpha(double* alpha1) {
    ...
    for (auto it = imu_map_factors.begin(); it != imu_map_factors.end(); ++it) {
        ...
        flower_vio::Vector15d model_residuals =
            imu_pre_factor->imu_jacobians_in[0] * Eigen::Map<flower_vio::Vector6d>(p0) +
            imu_pre_factor->imu_jacobians_in[1] * Eigen::Map<flower_vio::Vector9d>(p1) +
            imu_pre_factor->imu_jacobians_in[2] * Eigen::Map<flower_vio::Vector6d>(p2) +
            imu_pre_factor->imu_jacobians_in[3] * Eigen::Map<flower_vio::Vector9d>(p3) +
            imu_pre_factor->imu_jacobians_in[4] * Eigen::Map<flower_vio::Vector1d>(p4);
        int p_idx = 5;
        if (ESTIMATE_GLOBAL_POSE) {
            model_residuals +=  imu_pre_factor->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[O_POSGLOBAL]); p_idx++;
        }
        if (ESTIMATE_TD) {
            model_residuals +=  imu_pre_factor->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector1d>(rhs_posbias_save_pointer + order2p_local[O_TD]); p_idx++;
        }
        if (ESTIMATE_EXTRINSIC) {
            model_residuals +=  imu_pre_factor->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[O_EXTRINSIC]); p_idx++;
        }
        if (ESTIMATE_ACC_SCALE) {
            model_residuals +=  imu_pre_factor->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector3d>(rhs_posbias_save_pointer + order2p_local[O_ACC_S]); p_idx++;
        }
        if (ESTIMATE_GYR_SCALE) {
            model_residuals +=  imu_pre_factor->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector3d>(rhs_posbias_save_pointer + order2p_local[O_GYR_S]); p_idx++;
        }
        alpha1[0] += model_residuals.squaredNorm();
    }
    ...
    for (auto it = idepth_map_factors_short.begin(); it != idepth_map_factors_short.end(); ++it) {
        ...
        for (int i = 1; i < (int)pindexs.size(); ++i) {
            ...
            flower_vio::Vector2d model_residuals =
                Eigen::Map<flower_vio::Matrix2_6d>(p0 + 0) * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[pindex2order[pindexs[0]]]) +
                Eigen::Map<flower_vio::Matrix2_6d>(p0 + 6 * 2) * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[pindex2order[pindexs[i]]]) +
                Eigen::Map<flower_vio::Vector2d>(p0 + 6 * 4) * Eigen::Map<flower_vio::Vector1d>(it->second->rhs_idepth_save);
            alpha1[0] += model_residuals.squaredNorm();
            v_index++;
        }
    }
    ...
    for (auto& it : long_feature_factors) {
        ...
        for (int i = 1; i < (int)pindexs.size(); ++i) {
            ...
            flower_vio::Vector2d model_residuals =
                Eigen::Map<flower_vio::Matrix2_6d>(p0 + 0) * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[pindex2order[pindexs[0]]]) +
                Eigen::Map<flower_vio::Matrix2_6d>(p0 + 6 * 2) * Eigen::Map<flower_vio::Vector6d>(rhs_posbias_save_pointer + order2p_local[pindex2order[pindexs[i]]]) +
                Eigen::Map<flower_vio::Vector2d>(p0 + 6 * 4) * Eigen::Map<flower_vio::Vector1d>(it->rhs_idepth_save);
            alpha1[0] += model_residuals.squaredNorm();
            v_index++;
        }
    }
...
void VisualInertialBase::EvaluateCost(double* cost, double* model_cost_change) {
    ...
    if (cost) {
        ...
        for (auto it1 = idepth_map_factors_short.begin(); it1 != idepth_map_factors_short.end(); it1++) {
            ...
            for (int i = 1; i < (int)pindexs.size(); ++i) {
                ...
                Eigen::Map<flower_vio::Vector2d>visual_residual_in(visual_residuals + v_index * 2);
    ...
    if (model_cost_change) {
        ...
        for (auto it = imu_map_factors.begin(); it != imu_map_factors.end(); ++it) {
            ...
            flower_vio::Vector15d model_residuals =
                imu_pre_factori->imu_jacobians_in[0] * Eigen::Map<flower_vio::Vector6d>(p0) +
                imu_pre_factori->imu_jacobians_in[1] * Eigen::Map<flower_vio::Vector9d>(p1) +
                imu_pre_factori->imu_jacobians_in[2] * Eigen::Map<flower_vio::Vector6d>(p2) +
                imu_pre_factori->imu_jacobians_in[3] * Eigen::Map<flower_vio::Vector9d>(p3) +
                imu_pre_factori->imu_jacobians_in[4] * Eigen::Map<flower_vio::Vector1d>(p4);
            int p_idx = 5;
            if (ESTIMATE_GLOBAL_POSE) {
                model_residuals += imu_pre_factori->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[O_POSGLOBAL]); p_idx++;
            }
            if (ESTIMATE_TD) {
                model_residuals += imu_pre_factori->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector1d>(inc_posbias_pointer + order2p_local[O_TD]); p_idx++;
            }
            if (ESTIMATE_EXTRINSIC) {
                model_residuals += imu_pre_factori->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[O_EXTRINSIC]); p_idx++;
            }
            if (ESTIMATE_ACC_SCALE) {
                model_residuals += imu_pre_factori->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector3d>(inc_posbias_pointer + order2p_local[O_ACC_S]); p_idx++;
            }
            if (ESTIMATE_GYR_SCALE) {
                model_residuals += imu_pre_factori->imu_jacobians_in[p_idx] * Eigen::Map<flower_vio::Vector3d>(inc_posbias_pointer + order2p_local[O_GYR_S]); p_idx++;
            }
        ...
        for (auto it = idepth_map_factors_short.begin(); it != idepth_map_factors_short.end(); ++it) {
            ...
            for (int i = 1; i < (int)pindexs.size(); ++i) {
                double* p0 = visual_jacobians + v_index * VRNUM * 2;
                flower_vio::Vector2d model_residuals =
                    Eigen::Map<flower_vio::Matrix2_6d>(p0 + 0) * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[pindex2order[pindexs[0]]]) +
                    Eigen::Map<flower_vio::Matrix2_6d>(p0 + 6 * 2) * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[pindex2order[pindexs[i]]]) +
                    Eigen::Map<flower_vio::Vector2d>(p0 + 6 * 4) * Eigen::Map<flower_vio::Vector1d>(inc_idepth_short_pointer + index);
                model_cost_change[0] += model_residuals.transpose() * (Eigen::Map<flower_vio::Vector2d>(visual_residuals_save + v_index * 2) - model_residuals / 2);
                Eigen::Map<flower_vio::Vector2d>(visual_model_residual_accum + v_index * 2) += model_residuals;
                v_index++;
            }
            index++;
        }
        ...
        for (auto& it : long_feature_factors) {
            ...
            for (int i = 1; i < (int)pindexs.size(); ++i) {
                ...
                flower_vio::Vector2d model_residuals =
                    Eigen::Map<flower_vio::Matrix2_6d>(p0 + 0) * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[pindex2order[pindexs[0]]]) +
                    Eigen::Map<flower_vio::Matrix2_6d>(p0 + 6 * 2) * Eigen::Map<flower_vio::Vector6d>(inc_posbias_pointer + order2p_local[pindex2order[pindexs[i]]]) +
                    Eigen::Map<flower_vio::Vector2d>(p0 + 6 * 4) * Eigen::Map<flower_vio::Vector1d>(inc_outside_pointer  + index);
                model_cost_change[0] += model_residuals.transpose() * (Eigen::Map<flower_vio::Vector2d>(visual_residuals_save + v_index * 2) - model_residuals / 2);
                Eigen::Map<flower_vio::Vector2d>(visual_model_residual_accum + v_index * 2) += model_residuals;
                v_index++;
            }
