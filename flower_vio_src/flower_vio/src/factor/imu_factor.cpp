...
#define LEN 6
    ...
    Eigen::Vector3d P0;
    Eigen::Quaterniond Q_WI_WC;


    if (ESTIMATE_GLOBAL_POSE) {
        P0 = Eigen::Vector3d (parameters[p_idx][0], parameters[p_idx][1], parameters[p_idx][2]);
        Q_WI_WC = Eigen::Quaterniond (parameters[p_idx][6], parameters[p_idx][3], parameters[p_idx][4], parameters[p_idx][5]);
        p_idx++;
    } else {
        P0.setZero();
        Q_WI_WC.setIdentity();
    ...
    if (jacobians) {
        ...
        if (jacobians[4]) {
            Eigen::Map<flower_vio::Vector15d > jacobian_scale(jacobians[4]);
        ...
        if (ESTIMATE_TD) {
            if (jacobians[j_idx]) {
                Eigen::Map<flower_vio::Vector15d > jacobian_dt(jacobians[j_idx]);
                jacobian_dt.setZero();
